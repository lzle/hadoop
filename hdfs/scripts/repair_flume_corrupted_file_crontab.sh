...
LOG_FILE="/var/log/repair_file/repair_file.log"
CURRENT_TIME=$(date)
...
extract_log() {
    tail -2000 /var/log/azshara_bytedance/azshara.log | grep "file is corrupted"| grep "error when read" \
       | awk '{print $(NF-5)}' | awk -F ',' '{print $1}' | sort | uniq > corrupted_files
}
